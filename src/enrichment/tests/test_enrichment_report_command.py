...
def _match(watch, date, summary, category, identifier):
    ...
    return Match.objects.create(
        ...
        ai_summary=summary, category=category,
    )
...
def test_report_names_the_clusters_that_split(watch):
    ...
    _match(watch, day, "Declarou de utilidade publica a entidade A", "regulation", "a1")
    _match(watch, day, "Declarou de utilidade publica a entidade B", "regulation", "a2")
    _match(watch, day, "Declarou de utilidade publica a entidade C", "other", "a3")
...
def test_report_ignores_matches_outside_the_window_and_unenriched_ones(watch):
    _match(watch, datetime.date(2026, 8, 20), "Fora da janela", "tender", "c1")
    _match(watch, datetime.date(2026, 8, 27), None, "", "c2")
...
def test_report_shows_the_signal_score_has_spread(watch):
    ...
    for i, score in enumerate([0, 1, 2, 3, 3]):
        m = _match(watch, day, f"Resumo {i}", "tender", f"d{i}")
